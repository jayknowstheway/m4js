...
function midiIn(input) {
    // format input to array with pitch and velocity  
...
function deviceOnOff(onOff) {
    //    onOff = 1;
    var nextDevicePath = removeQuotes(nextDevice.path);
    var pathArray = nextDevicePath.split(" ");
    // add param number
    pathArray.push('parameters', 0);
    //rebuild the string
    var newPath = pathArray.join(" ");
    var selectedParam = new LiveAPI(newPath);
    selectedParam.set('value', onOff);
    log("deviceOnOff", selectedParam.get('value'));
    ...
    switch (crud) {
        ...
        case 1: // "ReadNext"
            ...
            if (!UI) {
                ...
                // this part needs to save/read a variable and iterate through UI[deviceName][i]
                // (UI).push(UI.shift());
